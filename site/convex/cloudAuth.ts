...
export function clean(value: string, max = 200) {
  const text = value.trim();
  if (!text || text.length > max)
    fail(`Enter between 1 and ${max} characters.`);
  return text;
}
export async function actor(ctx: QueryCtx | MutationCtx, token?: string) {
  if (token) {
    ...
    const d = await ctx.db
      ...
      .unique();
    if (!d?.userId || !d.email || d.expires < Date.now())
      fail("Sign in to cloud again.");
    return { userId: d.userId, email: d.email, name: d.name || d.email };
  }
  const id = await ctx.auth.getUserIdentity();
  if (!id || !id.email || id.emailVerified !== true)
    fail("Sign in with a verified email address.");
  return {
    userId: id.subject,
...
export async function access(
  ...
  token?: string,
  required: "read" | "write" | "owner" = "read",
) {
  ...
  const member = await ctx.db
    ...
    .withIndex("by_project_user", (q) =>
      q.eq("projectId", projectId).eq("userId", user.userId),
    )
    .unique();
  if (
    !project ||
    project.archived ||
    !member ||
    (required === "write" && member.role === "reviewer") ||
    (required === "owner" && member.role !== "owner")
  )
