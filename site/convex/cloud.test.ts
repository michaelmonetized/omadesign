...
describe("Publishing and invitation boundaries", () => {
  it("only accepts invitations for the verified recipient", async () => {
    const { t, owner, reviewer } = setup();
    const projectId = await owner.mutation(api.projects.create, { title: "Invite" });
    const id = await t.run(ctx => ctx.db.insert("cloudInvites", { projectId, email: "reviewer@example.com", role: "reviewer", inviter: "owner", expires: Date.now() + 100000, accepted: false }));
    await expect(owner.mutation(api.projects.accept, { id })).rejects.toThrow();
    await reviewer.mutation(api.projects.accept, { id });
    expect((await reviewer.query(api.projects.get, { projectId })).role).toBe("reviewer");
    await expect(reviewer.mutation(api.projects.accept, { id })).rejects.toThrow();
  });
  it("requires owner publication and hides unpublished competition entries", async () => {
    const { t, owner, reviewer } = setup();
    const projectId = await owner.mutation(api.projects.create, { title: "Showcase" });
    const { snapshotId, sourceId, competitionId } = await t.run(async ctx => {
      ...
      const base = { projectId, storageId, uploader: "owner", name: "flat.png", size: 5, contentType: "image/png", created: Date.now(), version: 1 };
      return { snapshotId: await ctx.db.insert("cloudFiles", { ...base, kind: "snapshot" }), sourceId: await ctx.db.insert("cloudFiles", { ...base, kind: "source" }), competitionId: await ctx.db.insert("cloudCompetitions", { title: "Test", description: "Disposable", opens: Date.now()-1000, closes: Date.now()+100000, active: true }) };
    });
    expect(await t.query(api.showcase.list, {})).toEqual([]);
    await expect(reviewer.mutation(api.showcase.publish, { snapshotId, title: "Private", description: "No" })).rejects.toThrow();
    await expect(owner.mutation(api.showcase.publish, { snapshotId: sourceId, title: "Source", description: "No" })).rejects.toThrow();
    const showcaseId = await owner.mutation(api.showcase.publish, { snapshotId, title: "Public work", description: "Only the flat export" });
    await owner.mutation(api.showcase.enter, { showcaseId, competitionId });
    await expect(owner.mutation(api.showcase.enter, { showcaseId, competitionId })).rejects.toThrow();
  ...
  it("archives projects and restores only for the owner", async () => {
    ...
    const projectId = await owner.mutation(api.projects.create, { title: "Archive" });
    await owner.mutation(api.projects.archive, { projectId });
    await expect(owner.query(api.projects.get, { projectId })).rejects.toThrow();
    await expect(reviewer.mutation(api.projects.restore, { projectId })).rejects.toThrow();
    await owner.mutation(api.projects.restore, { projectId });
    expect((await owner.query(api.projects.get, { projectId })).title).toBe("Archive");
  });
});
