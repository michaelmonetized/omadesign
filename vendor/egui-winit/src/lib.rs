...
impl State {
    ...
    fn on_keyboard_input(&mut self, event: &winit::event::KeyEvent) {
        ...
        if let Some(active_key) = logical_key.or(physical_key) {
            if pressed
                && push_clipboard_command(
                    &mut self.egui_input.events,
                    active_key,
                    physical_key,
                    self.modifiers,
                    || self.clipboard.get(),
...
// Clipboard events carry no modifiers. Keep the originating press immediately
...
fn push_clipboard_command(
    ...
    let cut = is_cut_command(modifiers, key);
    let copy = is_copy_command(modifiers, key);
    let paste = is_paste_command(modifiers, key);
    if !(cut || copy || paste) {
        return false;
    }
    ...
    } else if let Some(contents) = read_clipboard() {
...
mod clipboard_chord_tests {
    use super::push_clipboard_command;
    use egui::{Event, Key, Modifiers};

    #[test]
    fn modified_paste_retains_press_even_without_clipboard_text() {
        for clipboard in [None, Some(String::new())] {
            let mut events = Vec::new();
            let modifiers = Modifiers::CTRL | Modifiers::COMMAND | Modifiers::SHIFT;
            ...
            // A modifier release in this same native frame must not erase the chord.
            events.push(Event::ModifiersChanged(Modifiers::NONE));
            assert!(matches!(&events[..], [Event::Key {
                key: Key::V, physical_key: Some(Key::V), pressed: true,
                modifiers: pressed_modifiers, ..
            }, Event::ModifiersChanged(Modifiers::NONE)] if *pressed_modifiers == modifiers));
        }
    }

    #[test]
    fn clipboard_payload_and_single_semantic_event_are_preserved() {
        let modifiers = Modifiers::CTRL | Modifiers::COMMAND;
        for (key, semantic) in [(Key::C, Event::Copy), (Key::X, Event::Cut)] {
            let mut events = Vec::new();
            assert!(push_clipboard_command(
                &mut events,
                key,
                None,
                modifiers,
                || { panic!("copy and cut must never read the clipboard") }
            ));
        ...
        assert_eq!(events.len(), 2);
        assert_eq!(events[1], Event::Paste("first\nsecond".into()));
        assert!(matches!(
            events[0],
            Event::Key {
                key: Key::V,
    ...
    #[test]
    fn ordinary_keys_do_not_read_or_emit_clipboard_events() {
        let mut events = Vec::new();
        assert!(!push_clipboard_command(
            &mut events,
            Key::V,
