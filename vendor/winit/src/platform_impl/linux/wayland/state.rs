...
use crate::platform_impl::wayland::seat::{
    DndSession, PointerConstraintsState, PointerGesturesState, RelativePointerState, TextInputState,
...
pub struct WinitState {
    ...
    /// Pointer gestures (trackpad pinch).
    ...
    /// In-flight Wayland file drop.
    pub dnd: Option<DndSession>,
...
impl WinitState {
    pub fn new(
        ...
        for seat in seat_state.seats() {
            let mut ss = WinitSeatState::new();
            if let Some(mgr) = &data_device_manager {
                ss.data_device = Some(mgr.get_data_device(queue_handle, &seat));
            }
            seats.insert(seat.id(), ss);
        }
