...
mod tests {
    ...
    fn copy(studio: &mut Studio) -> String {
        ...
        let output = ctx.run_ui(Default::default(), |ui| studio.copy_selection(ui.ctx()));
        output.platform_output.commands.into_iter().find_map(|command| match command {
    ...
    fn cross_window_rasters_and_shapes_keep_positions_with_one_undo() {
        ...
        let raster = Layer::placed_raster("Image", crate::document::Pixels::new(3, 2), Pt::new(98., 142.), Pt::new(60., 40.));
        ...
        assert_eq!(target.doc.find_shape(pasted.0, pasted.1).unwrap().geom.bbox(), bounds);
    ...
    fn external_text_replaces_stale_internal_clipboard_and_centers_after_pan_zoom() {
        ...
        copy(&mut studio);
        studio.canvas_rect = Some(Rect::from_min_size(Pos2::new(80., 40.), egui::vec2(1000., 600.)));
        studio.view.scale = 2.;
        ...
        let Geom::Text(text) = &shape.geom else { panic!("editable text expected") };
    ...
    fn delayed_paste_does_not_modify_another_tab() {
        ...
        studio.clipboard_jobs.push(PasteJob { receiver, owner: "different-tab".into() });
        tx.send(Ok(PasteContents::Internal("omadesign-shapes:[]".into()))).unwrap();
        let history = studio.history.len();
...
impl Studio {
    ...
    pub fn copy_selection(&mut self, ctx: &egui::Context) {
        ...
        for &(li, id) in &self.selection {
            ids.extend(crate::layout::descendants(&self.doc, li, id).into_iter().map(|id| (li, id)));
        }
        let mut shapes: Vec<Shape> = self.doc.layers.iter().enumerate().flat_map(|(li, layer)| {
            let ids = &ids;
            layer.kind.shapes().into_iter().flatten().filter(move |s| ids.contains(&(li, s.id))).cloned()
        }).collect();
        ...
        let rasters: Vec<_> = self.selection.iter().filter_map(|(li, id)| {
            (*id == RASTER_ID).then(|| self.doc.layers.get(*li).cloned()).flatten()
        }).collect();
        ...
        let payload = if rasters.is_empty() {
            serde_json::to_string(&shapes).map(|s| format!("{}{s}", Self::CLIP_PREFIX)).map_err(|e| e.to_string())
        } else {
            let mut doc = Document::new("Clipboard", self.doc.width, self.doc.height, self.doc.dpi);
            doc.layers.clear();
            if !shapes.is_empty() {
                let mut layer = Layer::vector("Copied objects");
                layer.kind = LayerKind::Vector { shapes: shapes.clone() };
                doc.layers.push(layer);
            }
            doc.layers.extend(rasters.iter().cloned().map(|mut l| { l.parent = None; l }));
    ...
    pub fn cut_selection(&mut self, ctx: &egui::Context) {
        ...
        if self.status.starts_with("copied ") {
            if !self.delete_focused_edit() { self.delete_objects(); }
    ...
    pub fn clipboard_center(&self) -> Pt {
        self.canvas_rect.filter(|r| r.is_positive()).map_or(self.doc.size() * 0.5, |r| {
            self.view.to_world(Pt::new(r.width() * 0.5, r.height() * 0.5))
        })
    }
    ...
    pub fn request_clipboard_paste(&mut self, ctx: &egui::Context, payload: Option<&str>) {
        ...
        std::thread::spawn(move || {
            let result = crate::clipboard::read().or_else(|error| {
                fallback.as_deref().map(crate::clipboard::parse_text).unwrap_or(Err(error))
            }).and_then(|content| {
                let content = match (content, fallback.as_deref()) {
                    (ClipboardContent::Empty, Some(text)) => crate::clipboard::parse_text(text)?,
                    (content, _) => content,
                };
        ...
        self.clipboard_jobs.push(PasteJob { receiver, owner: self.swap_id.clone() });
    ...
    pub(super) fn poll_clipboard_jobs(&mut self, ctx: &egui::Context) {
        while let Some(job) = self.clipboard_jobs.first() {
            let result = match job.receiver.try_recv() {
                ...
                Err(TryRecvError::Disconnected) => Err("The clipboard worker stopped unexpectedly".into()),
            };
            let job = self.clipboard_jobs.remove(0);
            if job.owner != self.swap_id {
                self.status = "The destination tab changed. Paste again in the intended tab.".into();
                continue;
            }
            match result {
                Ok(PasteContents::Internal(text)) => self.paste_clipboard(Some(&text)),
                Ok(PasteContents::External(prepared)) => {
                    if let Err(error) = self.insert_prepared_clipboard(prepared) { self.status = error; }
                }
                Err(error) => self.status = error,
            }
        }
        if !self.clipboard_jobs.is_empty() { ctx.request_repaint_after(Duration::from_millis(50)); }
    }
    ...
    pub fn paste_clipboard(&mut self, payload: Option<&str>) {
        let parsed = match payload {
            Some(text) if text.starts_with(Self::CLIP_PREFIX) => {
                serde_json::from_str::<Vec<Shape>>(&text[Self::CLIP_PREFIX.len()..])
                    .map(|shapes| (shapes, vec![])).map_err(|e| format!("Invalid copied objects: {e}"))
            }
            Some(text) if text.starts_with(Self::OBJECT_CLIP_PREFIX) => {
                crate::project::decode(&text[Self::OBJECT_CLIP_PREFIX.len()..]).map(|doc| {
                    ...
                    for layer in doc.layers {
                        if let Some(items) = layer.kind.shapes() { shapes.extend(items.iter().cloned()); }
                        else { rasters.push(layer); }
                    }
                    (shapes, rasters)
                })
            }
            Some(text) => {
                let result = crate::clipboard::parse_text(text)
                    .and_then(|content| self.insert_clipboard_content(content, self.clipboard_center()));
        ...
        if shapes.is_empty() && rasters.is_empty() { self.status = "clipboard is empty".into(); return; }
        ...
        if !shapes.is_empty() {
            let li = self.vector_target().unwrap_or_else(|| {
                ...
                next_layer += 1;
                commands.push(Cmd::AddLayer { index, layer: Layer::vector("Copied objects") });
                index
            });
            let remap: HashMap<_, _> = shapes.iter().map(|s| (s.id, crate::document::next_id())).collect();
            for mut shape in shapes {
                ...
                if shape.layout.parent.is_none() { selection.push((li, shape.id)); }
        ...
        for mut layer in rasters {
            ...
            commands.push(Cmd::AddLayer { index: next_layer, layer });
            next_layer += 1;
        }
